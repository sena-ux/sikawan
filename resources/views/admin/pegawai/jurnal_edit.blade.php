            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden card-hover fade-in">
                ...
                <div class="p-6 sm:p-8">
                    <form id="leaveForm" class="space-y-5" enctype="multipart/form-data" method="post"
                        action="{{ route('jurnal.update', encrypt($jurnal->id)) }}">
                        @csrf
                        @method('PUT')
                        <!-- Hari Field -->
                        <div class="fade-in">
                            <label for="hari" class="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                ...
                                <option value="senin" {{ $jurnal->hari == 'senin' ? 'selected' : '' }}>Senin</option>
                                <option value="selasa" {{ $jurnal->hari == 'selasa' ? 'selected' : '' }}>Selasa</option>
                                <option value="rabu" {{ $jurnal->hari == 'rabu' ? 'selected' : '' }}>Rabu</option>
                                <option value="kamis" {{ $jurnal->hari == 'kamis' ? 'selected' : '' }}>Kamis</option>
                                <option value="jumat" {{ $jurnal->hari == 'jumat' ? 'selected' : '' }}>Jumat</option>
                                <option value="sabtu" {{ $jurnal->hari == 'sabtu' ? 'selected' : '' }}>Sabtu</option>
                                <option value="minggu" {{ $jurnal->hari == 'minggu' ? 'selected' : '' }}>Minggu</option>
                            </select>
                        </div><!-- Tanggal Field -->
                        <div class="fade-in">
                            <label for="tanggal" class="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                <svg class="w-4 h-4 mr-2 text-purple-600" fill="none" stroke="currentColor"
                                    viewbox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                ...
                                id="tanggal"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 bg-gray-50 hover:bg-white"
                                name="tanggal" value="{{ $jurnal->tanggal }}" required>
                        </div><!-- Kegiatan Field -->
                        <div class="fade-in">
                            <label for="kegiatan" class="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                <svg class="w-4 h-4 mr-2 text-purple-600" fill="none" stroke="currentColor"
                                    viewbox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                id="kegiatan"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 bg-gray-50 hover:bg-white"
                                placeholder="Masukan kegiatan anda" name="kegiatan" value="{{ $jurnal->kegiatan }}"
                                required>
                        </div><!-- Deskripsi Field -->
                        <div class="fade-in">
                            <label for="deskripsi" class="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                <svg class="w-4 h-4 mr-2 text-purple-600" fill="none" stroke="currentColor"
                                    viewbox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                required>{{ $jurnal->deskripsi }}</textarea>
                        ...
                        <div class="fade-in">
                            ...
                            <div class="file-upload-wrapper">
                                <input type="file" id="foto" name="foto" accept="image/*" class="file-upload-input"
                            ...
                            <div id="fileName" class="file-name">
                                @if($jurnal->foto)
                                    <span class="text-purple-600">File lama: <a href="{{ asset($jurnal->foto) }}"
                                            target="_blank">{{ basename($jurnal->foto) }}</a></span>
                                @endif
                            </div>
                        ...
                        <div class="pt-4">
                            <button type="submit" id="submitBtn"
                                ...
                                <span>Update Jurnal Harian</span>
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        $('#leaveForm').on('submit', function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    let msg = 'Terjadi kesalahan saat update jurnal.';
